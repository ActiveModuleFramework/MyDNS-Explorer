...
export class MainView extends JWF.Window {
  ...
  constructor(adapter: JWF.Adapter) {
    ...
    tree.addEventListener("itemSelect", e => {
      this.outputInfo(e.item.getItemValue() as [string, MyDNS.MyDNSInfo]);
    });
    tree.addEventListener("itemDblClick", async e => {
      const item = e.item;
      const id = (item.getItemValue() as string[])[0];
      const result = (await this.adapter.exec("MyDNS.getPassword", id)) as {
        pass: string;
      };
      if (result) {
   ...
   */
  async update() {
    const results = (await this.adapter.exec("MyDNS.getUsers")) as {
      [key: string]: unknown;
    }[];
    ...
    for (const key in infos) {
      ...
      if (pid === null) topInfos[key] = infos[key];
    }
    ...
    for (const key in topInfos) {
      ...
      const item = treeView.addItem(
        `${info.domainInfo.domainname} (${key})`,
        true
      );
   ...
   * @memberof MainView
   */
  private addTreeChild(
    item: JWF.TreeItem,
    info: MyDNS.MyDNSInfo,
    infos: { [keys: string]: MyDNS.MyDNSInfo }
  ) {
    const childInfo = info.childInfo;
    for (const cid of childInfo.masterid) {
      ...
      if (cinfo) {
        const citem = item.addItem(
          `${cinfo.domainInfo.domainname} (${cid})`,
          true
        );
  ...
  outputInfo(value: [string, MyDNS.MyDNSInfo]) {
    ...
    for (const index in domainInfo.type) {
      if (domainInfo.hostname[index])
        listView.addItem([
          "RECORD",
          domainInfo.hostname[index],
          domainInfo.type[index],
          domainInfo.content[index] || domainInfo.delegateid[index]
        ]);
    }
  }
}
