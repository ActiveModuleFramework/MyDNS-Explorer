    ...
    if (results) {
      ...
      for (const result of results) {
        const info = result.info as {
          domainInfo: { update: string; ipV4: string; domainname: string };
        };
        const update = info.domainInfo.update
          ? new Date(info.domainInfo.update).toLocaleString()
          : "";
        listView.addItem(
          [
            result.id as string,
            update,
            info.domainInfo.ipV4,
            info.domainInfo.domainname
          ],
          result
        );
      }
