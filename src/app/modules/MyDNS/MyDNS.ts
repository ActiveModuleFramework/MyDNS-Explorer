...
export class MyDNS extends amf.Module {
  ...
  async JS_update(): Promise<boolean> {
    ...
    if (!users || !users.isAdmin()) return false;
    const localDB = amf.Module.getLocalDB();
    const results = await localDB.all(
      "select mydns_id as id,mydns_password as pass from mydns"
    );
    ...
    for (const result of results) {
      ...
      const p = (async () => {
        ...
        if (!(await reader.getSession(id, pass))) return false;
        const info = await reader.getInfo();
        if (!info) return false;
        const result = await localDB.run(
          "update mydns set mydns_info=? where mydns_id=?",
          JSON.stringify(info),
          id
        );
  ...
  async JS_getPassword(id: string) {
    ...
    return await localDB.get(
      "select mydns_password as pass from mydns where mydns_id=?",
      id
    );
  }
  /**
  ...
  async JS_getUsers() {
    ...
    const localDB = amf.Module.getLocalDB();
    const results = await localDB.all(
      "select mydns_id as id,mydns_info as info from mydns"
    );
    for (const result of results) {
