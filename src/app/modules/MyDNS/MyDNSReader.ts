...
export class MyDNSReader{
	...
	async getChildInfo(): Promise<MyDNSChildInfo|null>{
		...
		for(let i=0;;i++){
			const idElement = doc.querySelector(`INPUT[NAME="CHILDINFO[masterid][${i}]"]`) as HTMLInputElement
			if (!idElement)
				break
			info.masterid[i] = idElement.value
			const domainElement = doc.querySelector(`INPUT[NAME="CHILDINFO[domainname][${i}]"]`) as HTMLInputElement
			info.domainname[i] = domainElement.value
			const ipv4Element = doc.querySelector(`INPUT[NAME="CHILDINFO[ipv4addr][${i}]"]`) as HTMLInputElement
			info.ipv4addr[i] = ipv4Element.value
			const ipv6lement = doc.querySelector(`INPUT[NAME="CHILDINFO[ipv6addr][${i}]"]`) as HTMLInputElement
			info.ipv6addr[i] = ipv6lement.value
			const updateElement = doc.querySelector(`INPUT[NAME="CHILDINFO[iplastdate][${i}]"]`) as HTMLInputElement
	...
	private static getParams(dom: JSDOM) : null|MyDNSDomainInfo{
		try {
			...
			const domainName = doc.querySelector('INPUT[name="DNSINFO[domainname]"]') as HTMLInputElement
			params.domainname = domainName.value
			for (let i = 0; ; i++) {
				const mx = doc.querySelector(`INPUT[name="DNSINFO[mx][${i}]"]`) as HTMLInputElement
				if (!mx)
					break
				params.mx[i] = mx.value
				const prio = doc.querySelector(`SELECT[name="DNSINFO[prio][${i}]"]`) as HTMLInputElement
				params.prio[i] = parseInt(prio.value)
			}
			for (let i = 0; ; i++) {
				const hostname = doc.querySelector(`INPUT[name="DNSINFO[hostname][${i}]"]`) as HTMLInputElement
				if (!hostname)
					break
				params.hostname[i] = hostname.value
				const type = doc.querySelector(`SELECT[name="DNSINFO[type][${i}]"]`) as HTMLInputElement
				params.type[i] = type.value
				const content = doc.querySelector(`INPUT[name="DNSINFO[content][${i}]"]`) as HTMLInputElement
				params.content[i] = content.value
				const delegateid = doc.querySelector(`SELECT[name="DNSINFO[delegateid][${i}]"]`) as HTMLInputElement
