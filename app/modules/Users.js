...
const AmfModule_1 = require("active-module-framework/AmfModule");
function getSHA256(v1, v2) {
    return crypto.createHash('sha256').update(v1 + (v2 ? v2 : '')).digest('hex');
}
class Users extends AmfModule_1.AmfModule {
    static async onCreateModule() {
        const localDB = AmfModule_1.AmfModule.getLocalDB();
    ...
    async onStartSession() {
        ...
        if (!user)
            user = this.logout();
        this.userInfo = user;
        AmfModule_1.AmfModule.output('ユーザ: %s', JSON.stringify(this.userInfo));
    }
    async isLogin(userId, userPass, local) {
        const localDB = AmfModule_1.AmfModule.getManager().getLocalDB();
        if (local) {
    ...
    static async getUserInfoFromNo(no, local) {
        if (local) {
    ...
    async JS_login(userId, userPass, local, keep) {
    ...
    async JS_setUser(userNo, userId, userName, userPass, local) {
        if (!this.isAdmin())
            return false;
        if (local) {
            ...
            let result;
            if (userNo == 0) {
                result = await localDB.run("insert into users values(null,1,?,?,?,'{}')", userId, pass, userName);
            }
            else {
                //作成したシリアル番号を返す
                result = await localDB.run("update users set users_id=?,users_password=?,users_name=? where users_no=?", userId, pass, userName, userNo);
            }
            return result;
        }
        return false;
    }
    async JS_delUser(userNo, local) {
        if (!this.isAdmin())
            return false;
        if (local) {
            const localDB = Users.getLocalDB();
